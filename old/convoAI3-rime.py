...
def create_agent_payload(
    ...
    greeting=None,
    rime_speaker=None,
    rime_sampling_rate=None,
    rime_speed_alpha=None,
    graph_id=None,
    debug_mode=False
):
    """
    Creates the payload for the agent to be sent to the Agora RTC channel with Rime TTS
    ...
    Args:
        ...
        rime_speaker: Rime speaker (defaults to constants["RIME_SPEAKER"])
        rime_sampling_rate: Rime sampling rate (defaults to constants["RIME_SAMPLING_RATE"])
        rime_speed_alpha: Rime speed alpha (defaults to constants["RIME_SPEED_ALPHA"])
    ...
    if rime_speaker is None:
        rime_speaker = constants["RIME_SPEAKER"]
    if rime_sampling_rate is None:
        rime_sampling_rate = constants["RIME_SAMPLING_RATE"]
    if rime_speed_alpha is None:
        rime_speed_alpha = constants["RIME_SPEED_ALPHA"]

    # Convert Rime parameters to appropriate types
    try:
        rime_sampling_rate = int(rime_sampling_rate)
        rime_speed_alpha = float(rime_speed_alpha)
        ...
        ("properties", {
            ...
            "tts": {
                ...
                "params": {
                    ...
                    "modelId": constants["RIME_MODEL_ID"],
                    "lang": constants["RIME_LANG"],
                    "samplingRate": rime_sampling_rate,
                    "speedAlpha": rime_speed_alpha
                }
            }
        })
    ])
