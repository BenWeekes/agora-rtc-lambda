...
Migration applied vs. the original file:
  ...
  - added properties.turn_detection with the new nested structure:
        mode: "default"
        config.start_of_speech.vad_config.{interrupt_duration_ms, prefix_padding_ms}
        config.end_of_speech.vad_config.{silence_duration_ms}
    end_of_speech.mode is "semantic" when ENABLE_AIVAD is true, else "vad",
...
def initialize_constants(profile=None):
    ...
    constants = {
        ...
        # MIGRATION: new turn_detection tunables
...
def create_agent_payload(channel, agent_token, prompt, greeting, failure_message, max_history,
    ...
    Args:
        ...
        vad_silence_duration: VAD silence duration in ms (end_of_speech)
        enable_bhvs: Enable behaviors
        enable_rtm: Enable RTM
        enable_aivad: Enable AI VAD - now selects end_of_speech semantic mode
    ...
    if tts_vendor == "rime":
        ...
        # FIXED: Use correct ElevenLabs format for Agora
        tts_config["params"] = {
            "key": constants["TTS_KEY"],
            "model_id": constants["TTS_MODEL"] if constants["TTS_MODEL"] else "eleven_flash_v2_5",
    ...
    # MIGRATION: build the new nested turn_detection object.
    # Replaces the deprecated properties.vad block and
    # advanced_features.enable_aivad flag.
    turn_detection_config = {
        ...
        "config": {
            ...
            "end_of_speech": {
                # semantic mode preserves the previous enable_aivad behaviour
    ...
    payload_items.append(("properties", OrderedDict([
        ...
        ("advanced_features", {
            "enable_bhvs": enable_bhvs,
            "enable_rtm": enable_rtm
            # MIGRATION: enable_aivad removed - deprecated. Its behaviour now
            # drives turn_detection.config.end_of_speech.mode above.
        }),
        ("enable_string_uid", False),
        ("idle_timeout", int(idle_timeout)),
        ("llm", llm_config),
        # MIGRATION: the deprecated ("vad", {...}) block was here. It is
        # replaced by the turn_detection object below.
